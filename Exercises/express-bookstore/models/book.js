...
const bookSchema = require("../resources/bookSchema.json");
const bookSchemaUpdate = require("../resources/bookSchemaUpdate.json");
...
class Book {
  ...
  static async update(isbn, data) {
    ...
    let dataValues = [...Object.values(data),isbn];
    let queryString = "";
    for(let i = 0;i<dataKeys.length;i++){
      if(i === dataKeys.length-1){
        queryString += `${dataKeys[i]}=($${i+1}) WHERE isbn=$${i+2}`;
      }else{
        queryString += `${dataKeys[i]}=($${i+1}), `;
      }
    }

    const result = await db.query(
      `UPDATE books SET ${queryString}
        RETURNING isbn,
                  amazon_url,
                  author,
                  language,
  ...
  static validate(book,updateMode=false) {
    let schema;
    updateMode ? schema = bookSchemaUpdate : schema = bookSchema;
    const result = jsonschema.validate(book,schema);
    if(!result.valid){
