...
describe("GET /companies", function () {
  ...
  test("Test filtering feature.", async function () {
    ...
    resp = await request(app).get("/companies?minEmployees=2&maxEmployees=2");    
    expect(resp.body).toEqual({
      companies:
          [
            {
    ...
    resp = await request(app).get("/companies?name=x&minEmployees=2");    
    expect(resp.body).toEqual({
      companies:
          [],
    });
  ...
  test("Test filtering feature errors.", async function () {
    let resp = await request(app).get("/companies?minEmployees=2&maxEmployees=1");
    expect(resp.status).toBe(400)
    expect(JSON.parse(resp.text).error.message).toEqual("Invalid query value: minEmployees must be less than maxEmployees")    

    resp = await request(app).get("/companies?minEmployees=2&invalidQuery=5");
    expect(resp.status).toBe(400)
